...
from examples.models.request import RankingRequest, FacebookEngagements, RedditEngagements, TwitterEngagements, ContentItem, Session
...
def process_twitter(data_file=TWITTER_DATA_FILE, num_samples=-1, seed=0) -> list[ContentItem]:
    ...
    df['simulated_replies'] = round((df['followers_count'] * 0.001) + np.random.normal(loc=0, scale=noise_std, size=len(df)), 0).clip(lower=0).astype(int)
    np.random.seed(repost_seed)
    df['simulated_reposts'] = round((df['followers_count'] * 0.005) + np.random.normal(loc=0, scale=noise_std, size=len(df)), 0).clip(lower=0).astype(int)
    np.random.seed(like_seed)
    df['simulated_likes'] = round((df['followers_count'] * 0.01) + np.random.normal(loc=0, scale=noise_std, size=len(df)), 0).clip(lower=0).astype(int)
